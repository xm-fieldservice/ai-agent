...
import { ConflictResolutionService, ConflictType, ConflictResolutionStrategy } from '../conflict-resolution-service';
...
vi.mock('../storage-service', () => {
    const mockStorageService = {
        ...
        getSetting: vi.fn().mockImplementation((key) => {
            if (key === 'pendingConflicts')
                return Promise.resolve([]);
            if (key === 'defaultConflictStrategy')
...
describe('ConflictResolutionService', () => {
    ...
    describe('冲突管理', () => {
        it('应该成功添加冲突', async () => {
            const conflict = {
                ...
                clientVersion: { id: 'msg1', content: 'client version', featureId: 'test', timestamp: 100 },
                serverVersion: { id: 'msg1', content: 'server version', featureId: 'test', timestamp: 200 }
            };
    ...
    describe('冲突解决策略', () => {
        it('应该设置默认解决策略', async () => {
            ...
            expect(mockStorageService.saveSetting).toHaveBeenCalledWith('defaultConflictStrategy', ConflictResolutionStrategy.CLIENT_WINS);
            // 检查冲突状态
    ...
    describe('自动冲突检测', () => {
        it('应该检测消息冲突', async () => {
            // 模拟本地和服务器消息
            const localMessage = { id: 'msg1', content: 'local', status: 'sent', featureId: 'test' };
            const serverMessage = { id: 'msg1', content: 'server', status: 'received', featureId: 'test' };
            // 模拟服务器响应
            mockNetworkService.request.mockResolvedValueOnce({
                msg1: serverMessage
            });
            // 模拟本地数据获取
            mockStorageService.getMessage.mockResolvedValueOnce(localMessage);
            // 检测冲突
    ...
    describe('自动冲突解决', () => {
        it('应该自动解决所有冲突', async () => {
            ...
            await conflictResolutionService.addConflict({
                id: 'conflict1',
                type: ConflictType.MESSAGE_CONTENT,
                clientVersion: { id: 'msg1' },
                serverVersion: { id: 'msg1' }
            });
            await conflictResolutionService.addConflict({
                id: 'conflict2',
                type: ConflictType.CONVERSATION_DATA,
                clientVersion: { id: 'conv1' },
                serverVersion: { id: 'conv1' }
            });
            // 设置时间戳优先策略
