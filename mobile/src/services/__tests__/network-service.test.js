...
describe('NetworkService', () => {
    ...
    beforeEach(() => {
        ...
        vi.spyOn(networkService, 'executeRequest').mockImplementation(async (task) => {
            // 根据请求决定返回不同的结果
            if (task.config.url === '/test1')
                return 1;
            if (task.config.url === '/test2')
                return 2;
            if (task.config.url === '/test3')
                return 3;
            return {};
        });
        // 处理任务时直接更新结果
        vi.spyOn(networkService, 'processTask').mockImplementation(async function (task) {
    ...
    describe('请求优先级', () => {
        it('应该按优先级处理请求', async () => {
            ...
            // 设置请求队列
            networkService.requestQueue = [task1, task2, task3];
            // 调用排序方法
            networkService.sortQueue();
            // 验证排序后的顺序
            const ids = networkService.requestQueue.map((task) => task.config.testId);
    ...
    describe('错误处理', () => {
        it('应该处理网络错误', async () => {
            // 重置之前的模拟
            vi.spyOn(networkService, 'executeRequest').mockRestore();
            vi.spyOn(networkService, 'processTask').mockRestore();
    ...
    describe('队列状态', () => {
        it('应该正确报告队列状态', async () => {
            // 禁用队列处理以便于测试
            vi.spyOn(networkService, 'processQueue').mockImplementation(() => Promise.resolve());
            // 添加请求到队列
            networkService.request({ url: '/test1' });
            networkService.request({ url: '/test2' });
            // 访问内部队列以确认请求已添加
            expect(networkService.requestQueue.length).toBe(2);
            // 检查队列状态报告
            ...
            networkService.clearQueue();
            // 确认队列已清空
            expect(networkService.requestQueue.length).toBe(0);
